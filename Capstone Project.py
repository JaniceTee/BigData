...
print(f"Number of columns in dataset after cleaning: {num_columns}")

# EXPLORATORY DATA ANALYSIS
# 1.Visualization of Target Variable (Diagnosed By a Medical Professional)
mental_illness = df['mh_diagnoses_professional'].value_counts()
print('\nMental Illness Prediction:', '\n', mental_illness)
sns.countplot(x='mh_diagnoses_professional', data=df, palette='hls')
plt.title('Diagnosed With a Mental Illness by a Medical Professional')
plt.show()

# 2.Ratio of Medically Diagnosed to Not Medically Diagnosed
count_not_diagnosed = len(df[df['mh_diagnoses_professional'] == 'No'])
count_medically_diagnosed = len(df[df['mh_diagnoses_professional'] == 'Yes'])
pct_of_not_diagnosed = count_not_diagnosed/(count_not_diagnosed+count_medically_diagnosed)
print('\npercentage of people not diagnosed with a M.H disorder by a medical professional is', pct_of_not_diagnosed*100)
pct_of_medically_diagnosed = count_medically_diagnosed/(count_not_diagnosed+count_medically_diagnosed)
print('percentage of people diagnosed with a M.H disorder by a medical professional is', pct_of_medically_diagnosed*100)

# 3.Description of Predictors
predictors = df.drop('mh_diagnoses_professional', axis=1)
print('\nDescription of Predictors: ', '\n', predictors.describe())

# 3B. Summary descriptive statistics shows an outlier in age
# Plotting Age Outlier
...
# Visualization of State of Abode
value_counts = df['live_us_territory'].value_counts()
value_counts.plot(kind='bar')
plt.xticks(rotation=90)
plt.title('Histogram of State of Abode')
plt.xlabel('State')
plt.ylabel('Frequency')
plt.tight_layout()
plt.show()

# Visualization of State of Work
value_counts = df['work_us_territory'].value_counts()
value_counts.plot(kind='bar')
plt.xticks(rotation=90)
plt.title('Bar Graph of State of Work')
plt.xlabel('State')
plt.ylabel('Frequency')
plt.tight_layout()
plt.show()

# Visualization of Tech Company
print(df['tech_comp'].unique())
# Tech Company contains 'unknown' so it has to be encoded
# 0 - No, 1 - Yes, 2 - Unknown
df['tech_comp'].replace(to_replace=['unknown'], value=2, inplace=True)
df['tech_comp'].replace(to_replace=['1.0'], value=1, inplace=True)
df['tech_comp'].replace(to_replace=['0.0'], value=0, inplace=True)
print(df['tech_comp'].unique())
# Plot Pie Chart
labels = df['tech_comp'].value_counts().index
sizes = df['tech_comp'].value_counts()
plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=140)
plt.title('Current Mental Illness Prevalence - Tech vs Non-Tech Industry (0 - No, 1 - Yes, 2 - Unknown)')
plt.show()

# Visualization of Self-Employed
print(df['self_emp'].unique())
# Plot Pie Chart
labels = df['self_emp'].value_counts().index
sizes = df['self_emp'].value_counts()
plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=140)
plt.title('Current Mental Illness Prevalence - Self-Employed vs Non-Self Employed (0 - No, 1 - Yes)')
plt.show()

# Visualize Remote Work
print(df['remote_work'].unique())
# Plot Pie Chart
labels = df['remote_work'].value_counts().index
sizes = df['remote_work'].value_counts()
plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=140)
plt.title('Current Mental Illness Prevalence - Remote Workers vs Non-Remote Workers (Do you work remotely?)')
plt.show()

# Visualize Number of Employees in Current Company
print(df['no_of_emp_in_comp'].unique())
...
# Plot Pie Chart
labels = df['no_of_emp_in_comp'].value_counts().index
sizes = df['no_of_emp_in_comp'].value_counts()
plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=140)
plt.title('Number of Employees in Current Company')
plt.show()
...
print("Frequency of Unique Values in 'medically_diagnosed_condition' variable:")
print(value_counts)
value_counts.plot(kind='bar')
plt.xticks(rotation=90)
...
columns_to_encode = ['live_us_territory', 'work_us_territory', 'remote_work', 'no_of_emp_in_comp', 'emp_mh_benefits',
                     'emp_mh_care', 'emp_mh_discussion', 'emp_mh_resources', 'anonymity_protection', 'mh_leave_request_ease',
                     'mh_conseq_coworkers', 'mh_discussion_neg_impact', 'ph_discussion_neg_impact', 'mh_discussion_cowork',
                     'mh_discussion_supervisor', 'mh_eq_ph_employer', 'mh_bad_response_workplace', 'mh_for_others_bad_response_workplace',
...
variables = df[['live_us_territory', 'work_us_territory', 'remote_work', 'no_of_emp_in_comp', 'emp_mh_benefits',
                     'emp_mh_care', 'emp_mh_discussion', 'emp_mh_resources', 'anonymity_protection', 'mh_leave_request_ease',
                     'mh_conseq_coworkers', 'mh_discussion_neg_impact', 'ph_discussion_neg_impact', 'mh_discussion_cowork',
                     'mh_discussion_supervisor', 'mh_eq_ph_employer', 'mh_bad_response_workplace', 'mh_for_others_bad_response_workplace',
                     'age', 'sex', 'self_emp', 'tech_comp', 'mh_diagnoses_professional']]
print('\nCorrelation Matrix: ', '\n', variables.corr())
...
x = df[['emp_mh_benefits', 'emp_mh_care', 'emp_mh_discussion', 'emp_mh_resources', 'mh_conseq_coworkers',
        'mh_eq_ph_employer', 'mh_bad_response_workplace', 'sex']].values
